...
{
    /**
     * Test class for emoji integration (place stones on emoji)
    ...
    public class EmojiTest
    {
        ...
        private Stone _currentStone, _firstStone;
...
//            Object.FindObjectOfType<StartCountdownTime>().OnCountdownEnd();
         ...
         */
        [UnityTest]
        public IEnumerator EmojiTestPlayer1()
        {
            _currentPlayer = 1;
         ...
         */
        [UnityTest]
        public IEnumerator EmojiTestPlayer2()
        {
            _currentPlayer = 2;
            yield return TestPlayer();
        }
         ...
         */
        [UnityTest]
        public IEnumerator EmojiTestPlayer3()
        {
            _currentPlayer = 3;
            yield return TestPlayer();
        }
         ...
         */
        [UnityTest]
        public IEnumerator EmojiTestPlayer4()
        {
            _currentPlayer = 4;
            yield return TestPlayer();
        }
        ...
        private IEnumerator TestPlayer()
        {
            ...
            yield return MoveToAnotherEmoji();
            yield return MoveToEmoji();
            yield return MoveToAnotherEmojiFromScoreArea();
            _firstStone = _currentStone;
            yield return GetStone();
            InitTest();
            yield return MoveToWorkshopSlot();
            CheckFirstStoneTouch(true);
            yield return MoveToEmoji();
            CheckFirstStoneTouch(false);
            yield return MoveCurrentStoneToWorkshop();
        }

        /**
         * Check, if first stone is touchable
         * @param touchable assert parameter
         */
        private void CheckFirstStoneTouch(bool touchable)
        {
            Assert.AreEqual(StoneIsTouchable(_firstStone), touchable,
                $"Player {_currentPlayer}: Unexpected touchable state at stone");
        }

        /**
         * Check, if specified stone is touchable and return true if so
         * @param stone stone to check
         * @return true, if touchable
         */
        private bool StoneIsTouchable(Stone stone)
        {
            var leanSelectable = stone.gameObject.GetComponent<LeanSelectable>();
            return leanSelectable.enabled;
        }

        /**
         * Moves current stone from emoji to workshop slot and checks, if successful
         */
        private IEnumerator MoveCurrentStoneToWorkshop()
        {
            // ReSharper disable once Unity.InefficientPropertyAccess
            yield return AssertMovement(_workshopSlot.transform.position + Vector3.up + Vector3.left,
                _workshopSlot.transform.position,
                $"Player {_currentPlayer}: Current Stone can't be removed from emoji");
        }

        /**
         * Move current stone from score area to emoji of another team and check conditions
         */
        private IEnumerator MoveToAnotherEmojiFromScoreArea()
        {
            // ReSharper disable once Unity.InefficientPropertyAccess
            yield return AssertMovement(_anotherScoreArea.transform.position + Vector3.up + Vector3.left,
                _scoreArea.transform.position,
                $"Player {_currentPlayer}: Stone can be placed on another emoji or stone not centered");
        }

        /**
         * Move current stone to emoji of another team and check conditions
         */
        private IEnumerator MoveToAnotherEmoji()
        {
            // ReSharper disable once Unity.InefficientPropertyAccess
            yield return AssertMovement(_anotherScoreArea.transform.position + Vector3.up + Vector3.left,
                _currentStone.transform.position,
                $"Player {_currentPlayer}: Stone can be placed on another emoji or stone not centered");
        }

        /**
         * Move current stone to emoji and check conditions
         */
        private IEnumerator MoveToEmoji()
        {
            // ReSharper disable twice Unity.InefficientPropertyAccess
            var rotationBefore = _currentStone.gameObject.transform.rotation;
            var scaleBefore = _currentStone.gameObject.transform.localScale;

            // ReSharper disable once Unity.InefficientPropertyAccess
            yield return AssertMovement(_scoreArea.transform.position + Vector3.up + Vector3.left,
                _scoreArea.transform.position + Vector3.up + Vector3.left,
                $"Player {_currentPlayer}: Stone is not in score area");
            Assert.IsTrue(_scoreArea.GetComponent<ScoreArea.ScoreArea>().ContainsStone(_currentStone),
                $"Player {_currentPlayer}: Stone not in score area");

            // ReSharper disable twice Unity.InefficientPropertyAccess
            var rotationAfter = _currentStone.gameObject.transform.rotation;
            var scaleAfter = _currentStone.gameObject.transform.localScale;

            Assert.AreEqual(rotationBefore, rotationAfter, $"Player {_currentPlayer}: stone rotation changed");
            Assert.AreEqual(scaleBefore, scaleAfter, $"Player {_currentPlayer}: stone scale changed");
        }

        /**
         * Test, if initial conditions are correct. Stone is expected to be in inventory
         */
        private void InitTest()
        {
            Assert.IsTrue(StoneIsTouchable(_currentStone),
                $"Player {_currentPlayer}: Stone is not selectable in the inventory");
            Assert.IsTrue(_workshopSlot.GetComponent<Workshop>().IsEmpty(),
                $"Player {_currentPlayer}: workshop is already full");
            Assert.IsFalse(_scoreArea.GetComponent<ScoreArea.ScoreArea>().ContainsStone(_currentStone),
                $"Player {_currentPlayer}: Stone should be not in score area");
        }
        ...
        private IEnumerator MoveToWorkshopSlot()
        {
            ...
            Assert.IsFalse(_workshopSlot.GetComponent<Workshop>().IsEmpty(),
                $"Player {_currentPlayer}: Workshop slot should be full");
        }
        ...
        private void LoadPlayer()
        {
            ...
            _scoreArea = GameObject.Find("Team_" + team + "/ScoreArea");

            var anotherTeam = team == 1 ? 2 : 1;
            _anotherScoreArea = GameObject.Find("Team_" + anotherTeam + "/ScoreArea");
